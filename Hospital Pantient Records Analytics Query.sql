...
SELECT
  ...
  TRY_CONVERT(datetime2, TRIM(e.START), 127) AS Start_DT,
  TRY_CONVERT(datetime2, TRIM(e.STOP), 127)  AS Stop_DT,
...
SELECT
  PatientId,
  Start_DT, Stop_DT,
  CASE WHEN Prev_Stop_DT IS NOT NULL AND DATEDIFF(DAY, Start_DT, Prev_Stop_DT) BETWEEN 1 AND 30 THEN 1 ELSE 0 END AS IsReadmit30d
FROM lagged;
GO
